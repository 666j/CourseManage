...
function showCourseDetail(data_info){
	...
	function handlerData(data){
		data = JSON.parse(data);
		$(".detail_cid").text(data.course_id);
		$(".detail_cname").text(data.course_name);
		$(".detail_credit").text(data.course_credit);
		$(".detail_teacherid").text(data.teacher_id);
		$(".detail_teachername").text(data.teacher_name);
		$(".detail_teachersex").text(data.teacher_sex);
		$(".detail_deptname").text(data.dept_name);
		$(".detail_classroom").text(data.room_id);
		$(".detail_acayear").text(data.acadmicyear );
		$(".detail_term").text(data.term);
		$(".detail_week").text(data. time_week);
		$(".detail_order").text(data.order_id);
		$(".detail_time").text(data.order_time);
		...
		$("#to_select_btn").on("click",function(){
			...
			$.ajax({
				...
				success:function(data){
					$("#show_course_detail").css("display","none");
						init2dimensional(data,mydata,"选课二维码");
				},
